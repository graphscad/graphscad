...
class Node_pulley(cdc.Dagnode):

    def getcomment(self):
        return 'this node is used to create a pulley'
    ...
    def getinputplugs(self):
        return [
                {'name': 'profile', 'value': 'MXL', 'type': 'str', 'combo' : ["MXL","40 D.P.","XL","H","T2.5","T5","T10","AT5","HTD 3mm","HTD 5mm","HTD 8mm","GT2 2mm","GT2 3mm","GT2 5mm"]},
                {'name': 'teeth', 'value': '22', 'type': 'str'},
                {'name': 'height', 'value': '1', 'type': 'str'},
                {'name': 'additional_tooth_width', 'value': '0.2', 'type': 'str'},
                {'name': 'additional_tooth_depth', 'value': '0', 'type': 'str'},
...
module pulley(profile,teeth=50,height=6,additional_tooth_width = 0.2,additional_tooth_depth=0)
{
    ...
    if ( profile == "MXL"      ) { ppulley (profile,teeth,height,pulley_tooth_spacing (teeth,2.032,0.254)                    , 0.508,additional_tooth_depth , 1.321,additional_tooth_width ); }
    if ( profile == "40 D.P."  ) { ppulley (profile,teeth,height,pulley_tooth_spacing (teeth,2.07264,0.1778)                 , 0.457,additional_tooth_depth , 1.226,additional_tooth_width ); }
    if ( profile == "XL"       ) { ppulley (profile,teeth,height,pulley_tooth_spacing (teeth,5.08,0.254)                     , 1.27 ,additional_tooth_depth , 3.051,additional_tooth_width ); }
    if ( profile == "H"        ) { ppulley (profile,teeth,height,pulley_tooth_spacing (teeth,9.525,0.381)                    , 1.905,additional_tooth_depth , 5.359,additional_tooth_width ); }
    if ( profile == "T2.5"     ) { ppulley (profile,teeth,height,pulley_tooth_spaceing_curvefit (teeth,0.7467,0.796,1.026)   , 0.7  ,additional_tooth_depth , 1.678,additional_tooth_width ); }
    if ( profile == "T5"       ) { ppulley (profile,teeth,height,pulley_tooth_spaceing_curvefit (teeth,0.6523,1.591,1.064)   , 1.19 ,additional_tooth_depth , 3.264,additional_tooth_width ); }
    if ( profile == "T10"      ) { ppulley (profile,teeth,height,pulley_tooth_spacing (teeth,10,0.93)                        , 2.5  ,additional_tooth_depth , 6.13 ,additional_tooth_width ); }
    if ( profile == "AT5"      ) { ppulley (profile,teeth,height,pulley_tooth_spaceing_curvefit (teeth,0.6523,1.591,1.064)   , 1.19 ,additional_tooth_depth , 4.268,additional_tooth_width ); }
    if ( profile == "HTD 3mm"  ) { ppulley (profile,teeth,height,pulley_tooth_spacing (teeth,3,0.381)                        , 1.289,additional_tooth_depth , 2.27 ,additional_tooth_width ); }
    if ( profile == "HTD 5mm"  ) { ppulley (profile,teeth,height,pulley_tooth_spacing (teeth,5,0.5715)                       , 2.199,additional_tooth_depth , 3.781,additional_tooth_width ); }
    if ( profile == "HTD 8mm"  ) { ppulley (profile,teeth,height,pulley_tooth_spacing (teeth,8,0.6858)                       , 3.607,additional_tooth_depth , 6.603,additional_tooth_width ); }
    if ( profile == "GT2 2mm"  ) { ppulley (profile,teeth,height,pulley_tooth_spacing (teeth,2,0.254)                        , 0.764,additional_tooth_depth , 1.494,additional_tooth_width ); }
    if ( profile == "GT2 3mm"  ) { ppulley (profile,teeth,height,pulley_tooth_spacing (teeth,3,0.381)                        , 1.169,additional_tooth_depth , 2.31 ,additional_tooth_width ); }
    if ( profile == "GT2 5mm"  ) { ppulley (profile,teeth,height,pulley_tooth_spacing (teeth,5,0.5715)                       , 1.969,additional_tooth_depth , 3.952,additional_tooth_width ); }
}
...
{
    echo (str("Belt type = ",profile,"; Number of teeth = ",teeth,"; Pulley Outside Diameter = ",pulley_OD,"mm "));
    ...
    difference()
    {
        ...
        
        {
            ...
            {
                if ( profile == "MXL"     ) { pulley_MXL(height);}
                if ( profile == "40 D.P." ) { pulley_40DP(height);}
                if ( profile == "XL"      ) { pulley_XL(height);}
                if ( profile == "H"       ) { pulley_H(height);}
                if ( profile == "T2.5"    ) { pulley_T2_5(height);}
                if ( profile == "T5"      ) { pulley_T5(height);}
                if ( profile == "T10"     ) { pulley_T10(height);}
                if ( profile == "AT5"     ) { pulley_AT5(height);}
                if ( profile == "HTD 3mm" ) { pulley_HTD_3mm(height);}
                if ( profile == "HTD 5mm" ) { pulley_HTD_5mm(height);}
                if ( profile == "HTD 8mm" ) { pulley_HTD_8mm(height);}
                if ( profile == "GT2 2mm" ) { pulley_GT2_2mm(height);}
                if ( profile == "GT2 3mm" ) { pulley_GT2_3mm(height);}
                if ( profile == "GT2 5mm" ) { pulley_GT2_5mm(height);}
            }
        }
    }
}
...
"""
    ...
    def pulley_tooth_spaceing_curvefit(self,teeth,b,c,d):
        return '((('+str(c)+'*pow('+teeth+','+str(d)+')) / ('+str(b)+'+pow('+teeth+','+str(d)+'))) * '+teeth+')'

    def pulley_tooth_spacing(self,teeth,tooth_pitch,pitch_line_offset):
        return '(2*(('+teeth+'*'+str(tooth_pitch)+')/(3.14159265*2)-'+str(pitch_line_offset)+'))'

    def pulley_radius(self,profile,teeth):
        d=1
        if ( profile == "MXL"      ): d=self.pulley_tooth_spacing(teeth,2.032,0.254)
        if ( profile == "40 D.P."  ): d=self.pulley_tooth_spacing(teeth,2.07264,0.1778)
        if ( profile == "XL"       ): d=self.pulley_tooth_spacing(teeth,5.08,0.254)
        if ( profile == "H"        ): d=self.pulley_tooth_spacing(teeth,9.525,0.381)
        if ( profile == "T2.5"     ): d=self.pulley_tooth_spaceing_curvefit(teeth,0.7467,0.796,1.026)
        if ( profile == "T5"       ): d=self.pulley_tooth_spaceing_curvefit(teeth,0.6523,1.591,1.064)
        if ( profile == "T10"      ): d=self.pulley_tooth_spacing(teeth,10,0.93)
        if ( profile == "AT5"      ): d=self.pulley_tooth_spaceing_curvefit(teeth,0.6523,1.591,1.064)
        if ( profile == "HTD 3mm"  ): d=self.pulley_tooth_spacing(teeth,3,0.381)
        if ( profile == "HTD 5mm"  ): d=self.pulley_tooth_spacing(teeth,5,0.5715)
        if ( profile == "HTD 8mm"  ): d=self.pulley_tooth_spacing(teeth,8,0.6858)
        if ( profile == "GT2 2mm"  ): d=self.pulley_tooth_spacing(teeth,2,0.254)
        if ( profile == "GT2 3mm"  ): d=self.pulley_tooth_spacing(teeth,3,0.381)
        if ( profile == "GT2 5mm"  ): d=self.pulley_tooth_spacing(teeth,5,0.5715)
        return '(('+d+')/2)'


    def evaluate(self):
        profile=self.getinputplugvalue('profile')
        teeth=self.getinputplugvalue('teeth')
        height=self.getinputplugvalue("height")
        additional_tooth_width=self.getinputplugvalue("additional_tooth_width")
        additional_tooth_depth=self.getinputplugvalue("additional_tooth_depth")
        ...
        code=code+'teeth='+teeth+','
        code=code+'height='+height+','
        code=code+'additional_tooth_width='+additional_tooth_width+','
        code=code+'additional_tooth_depth='+additional_tooth_depth
        code=code+');'
        ...
        self.setoutputplugvalue('radius', str(self.pulley_radius(profile,teeth)))
        return
